...
class SoftwareInventoryAnalyzer:
    ...
    def search_by_manufacturer(self, manufacturer: str, fetch_details: bool = True, max_hosts: int = 25) -> Dict:
        """
        ...
        for package in all_packages:
            ...
            if fetch_details and pkg_info['install_count'] > 0:
                # Limit hosts for performance (default 25, or custom max_hosts)
                installations = self._get_software_installations(package['sys_id'], limit=max_hosts)
    ...
    def search_by_software_name(self, software_name: str, fetch_details: bool = True, max_hosts: int = 25) -> Dict:
        """
        ...
        for package in packages:
            ...
            # Create a unique key for each package (combination of name + version + sys_id)
            # This handles cases where multiple packages might have the same version but different sys_ids
            package_key = f"{version}_{package['sys_id']}"
    ...
    def _get_software_installations(self, software_sys_id: str, limit: int = 25) -> List[Dict]:
        """
        Get installation details for a specific software package.
        ...
        # This table has more detailed information if accessible
        if not self.sam_access_denied:
            params = {
                ...
                'sysparm_order_by': 'install_date'  # Order by install date for consistency
            }
            ...
            if response and response.get('result'):
                for install in response['result']:
                    ...
                    if isinstance(host_ref, dict) and host_ref.get('value'):
                        ...
                        host_response = self.api_request(f'/api/now/table/cmdb_ci/{host_sys_id}')
                        
                        if host_response and host_response.get('result'):
                            host = host_response['result']
                            host_details = {
                                'host_sys_id': host_sys_id,
        ...
        if not installations:
            # Debug: Log fallback to relationships
            if self.debug:
            ...
            params = {
                ...
                'sysparm_order_by': 'sys_created_on'  # Order for consistency
            }
            ...
            if total_installs > 0:  # Only show software with actual installations
                ...
                for version, details in sorted(versions.items(), 
                    ...
                    if details['count'] > 0:
                        ...
                        # Show sample hosts only if there are 3 or fewer
                        if details['hosts'] and len(details['hosts']) <= 3:
                            print(f"        Hosts: {', '.join(details['hosts'][:3])}")
                        elif details['count'] > 25 and len(details['hosts']) > 0:
                            # If we have more than 25 installations, show it's a sample
                            print(f"        Sample: {len(details['hosts'])} of {details['count']} systems")
    ...
    def display_software_results(self, results: Dict):
        ...
        for version, details in sorted_versions:
            ...
            if details['hosts']:
                # Show up to 5 sample hosts
                total_hosts = len(details['hosts'])
                actual_install_count = details['install_count']
                
                # If we have more installations than hosts shown, indicate it's a sample
                if actual_install_count > total_hosts:
                    print(f"   Sample Hosts ({min(5, total_hosts)} of {actual_install_count} total):")
                else:
                    print(f"   Hosts ({min(5, total_hosts)} of {total_hosts}):")
                
                for host in details['hosts'][:5]:
                    print(f"      • {host['name']} ({host['ip']}) - {host['os']}")
                
                # Show ellipsis if there are more hosts than displayed
                if actual_install_count > 5:
...
def main():
    ...
    parser = argparse.ArgumentParser(
        description='ServiceNow Software Inventory Analyzer - Query installed software packages'
    )
    ...
    parser.add_argument('--no-details', action='store_true', help='Skip fetching host details entirely (fastest - counts only)')
    parser.add_argument('--full-details', action='store_true', help='Fetch ALL host details (slower - default limits to 25 hosts/version)')
    ...
    # Show info about the query mode
    if args.no_details:
        print("\n🚀 Running in fast mode: Counts only, no host enumeration")
    elif args.full_details:
        print("\n⚠️  Running in full details mode: This may be slow for software with many installations")
    else:
    ...
    if args.summary:
        ...
        max_hosts = 100 if args.full_details else 25
        results = analyzer.search_by_manufacturer(args.manufacturer, fetch_details, max_hosts)
        ...
        max_hosts = 100 if args.full_details else 25
        results = analyzer.search_by_software_name(args.software, fetch_details, max_hosts)
